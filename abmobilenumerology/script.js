document.addEventListener('DOMContentLoaded', () => {
    ...
    // 5. FAQ ACCORDION (legacy .faq-item)
    ...
    // 5b. FAQ V2 ACCORDION (isolated)
    const faqV2Items = document.querySelectorAll('.faq-v2-item');
    faqV2Items.forEach(item => {
        const trigger = item.querySelector('.faq-v2-q');
        if (!trigger) return;
        trigger.addEventListener('click', () => {
            const isOpen = item.classList.contains('open');
            faqV2Items.forEach(i => i.classList.remove('open'));
